...
Gallatin et al. \cite{gallatin1999trapeze} discovered \footnote{kap. 3.3 obr. 6} that if MTU is 1500 bytes, then interrupt handling accounts for 20\% - 25\% of receiver packet-processing overhead.
...
interrupts leads to frequent cache misses in both cache and inctruction caches.\footnote{kap. 4.2}\cite{cox2000profiling} 
...
However, its effectiveness is limited when performance is primarily constrained by memory access rather than computation.
...
The technique increases instructions per clock cycle by reducing memory access latency.
...
Because the processing logic in VPP is relatively stable, such predictions are often accurate and help to improve performance.
...
Additionally, inlining enables the compiler to perform more aggressive optimizations, such as removing unused code branches
...
As an extension of zero-copy via DMA, this reduces memory latency and can significantly lower RAM usage.
...
For example, a single binary can dynamically use AVX2-optimized code on supported processors, while falling back to compatible versions on older hardware.
